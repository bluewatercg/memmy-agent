...
main() {
  cd "$ROOT_DIR"
  package_log_init "mac-signed-arm64-$DESKTOP_VERSION-$PACKAGE_EDITION" "$ROOT_DIR/App/shell/desktop/release/logs"
  package_install_error_trap
  package_log "macOS signed package context: version=$DESKTOP_VERSION arch=arm64 edition=$PACKAGE_EDITION dmg=$DMG"
  ...
  echo "Signed arm64 DMG is ready:"
  echo "  $DMG"
}
