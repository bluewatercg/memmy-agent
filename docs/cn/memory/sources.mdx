---
title: Agent 来源与扫描
description: 了解 Memmy 如何扫描九种内置 Agent 的本地历史，以及安装的 Hook、插件、Skill 和实时记忆效果。
icon: ScanSearch
---

Agent Source 是 Memmy 读取外部 Agent 本地历史的适配器。来源扫描负责导入已经存在的对话；支持实时接入的 Agent 由 Hook 或插件负责新对话中的自动召回和采集。
...
<Callout type="warn" icon={false}>
  ...
  “同步新增”读取 Agent 已写入本地的数据；Hook 或插件负责新对话中的自动召回、自动采集和任务接续。只扫描不会修改 Agent 配置；安装 Hook、插件或 Skill 才会写入下文列出的配置和文件。
</Callout>
...
| OMP | `${PI_CODING_AGENT_SESSION_DIR:-~/.pi/agent/sessions}/**/*.jsonl` | 原生 Extension（Pi 兼容运行时） |
...
| WorkBuddy | `~/.workbuddy/projects/**/*.jsonl` | Skill（按需调用记忆） |
| Pi | `~/.pi/agent/sessions/**/*.jsonl` | Skill（按需调用记忆） |
| qwenwork | `~/.qwenworkcn/projects/**/*.jsonl` | Skill（按需调用记忆） |

跨 Agent 接入页面只展示当前检测到本地配置或历史来源的内置 Agent；没有检测到的内置 Agent 不会占用列表位置。
...
- 在 **高级操作 → 添加其他 Agent** 中，可以直接选择 `kimi code`、`zcode`、`minimax code`、`coder`，也可以输入其他 Agent 名称。
- 添加其他 Agent 时无需填写本地路径。Memmy 会启动一次发现任务，自动定位历史目录、识别消息格式、安装完整 `memmy-memory` Skill、导入最近 500 个完整对话回合，并保存后续同步使用的规则。此流程不会为未知 Agent 安装原生 Hook 或插件。
- 发现任务会先用可执行文件、`.app` 或已安装包的产品名称校验 Agent 身份。自动发现时，名称比较只忽略大小写、空格、连字符和下划线，不进行翻译、别名或语义猜测。如果找不到匹配的已安装产品，Memmy 会报告未找到并停止；用户可以继续提供安装产物、原生历史文件或目录以及 Skill 目录的绝对路径，让 Memmy 在这些限定路径内重新验证。Memmy 不会改用其他 Agent 或自己的历史。
- 自定义 Agent 在发现和验证期间只保存为内部草稿，不会显示在跨 Agent 列表中；验证出真实历史路径后才会显示。因而“未找到”的名称不会生成可见的 Agent 卡片，同时仍可在原任务中补充路径继续接入。
...
通过“添加其他 Agent”接入并由 Memmy 识别格式的 Agent 会保存首次导入的同步边界。后续同步只选择起始用户消息时间严格晚于该边界的完整回合，并继续依靠稳定消息 ID 和回合 ID 消除重复。
...
| 开关 | 效果 |
| --- | --- |
| 自动同步会话 | 自动从已接入的 Agent 采集新对话（启动时与定时），无需手动点「同步新增」 |
| 发现新 Agent 时自动接入 | 为新发现的内置 Agent 自动安装接入组件；精简 Skill 会随接入一并安装 |

关闭自动安装后，已检测到的 Agent 仍会出现在列表中，你可以手动点击 **安装 Hook**、**安装插件**或 **安装 Skill**。

## 数据处理与本地管理

- 每个成功写入的完整回合先成为一条 L1 原始记忆，再在后台生成摘要并建立索引。
- 可以查看本地数据路径（默认 `~/.memmy/memory-service`）、打开目录、导出 `memory.sqlite`，或在二次确认后清空本地记忆数据。
- 移除 Hook、插件或 Skill 不会删除已经导入的历史；清空本地数据是单独的显式操作。
...
| OMP | 原生 Extension | `before_agent_start`、`agent_settled`、`input` | 在 Pi 兼容运行时中请求前召回，只采集完全稳定的回合，并支持 `/memmy-resume` |
...
| FreeBuff | Skill | `memmy-memory` CLI | 扫描现有历史并提供按需记忆查询；当前不安装未验证的实时 Hook |
| WorkBuddy | Skill | Agent 主动调用 `memmy-memory` | 按需搜索、读取和写入共享记忆 |
| Pi | Skill | Agent 主动调用 `memmy-memory` | 按需搜索、读取和写入共享记忆 |
| qwenwork | Skill | Agent 主动调用 `memmy-memory` | 按需搜索、读取和写入共享记忆 |

### 回合开始时的权威项目上下文

Hook 或插件把当前 workspace 解析到项目后，`turn.start` 会先读取该项目已确认的上下文，再召回历史记忆。Memmy 将当前目标、目标约束、焦点工作项、下一步和验收标准放进独立的 `<memmy_project_context>` 区块，然后追加相关历史记忆。

两者职责不同：已批准的项目上下文是当前工作状态，L1/L2/L3/Skill 召回结果是历史证据；当前用户请求仍高于两者。项目边界由项目本身决定，不按 Agent 来源或 profile 切分，因此一个 CLI 批准的目标会自动提供给同一项目中的另一个 CLI；不同项目或 tenant 不会继承。

项目上下文治理在桌面端的**项目上下文包**对话框中完成。候选目标只有批准后才会注入，任一时刻只能有一个焦点工作项。
...
flowchart LR
  ...
  Start --> Authority["读取已确认项目上下文"]
  Authority --> Recall["检索相关历史记忆"]
  Recall --> Context["先注入项目上下文，再注入历史"]
...
- **权威项目上下文**：受支持的接入会先收到当前项目相同的已批准目标、约束和焦点，再收到历史召回。
- **自动召回**：Claude Code、Codex、OMP、OpenCode、OpenClaw 和 Hermes 会在正常请求执行前检索并注入相关历史记忆。
...
- **来源标记**：采集结果会记录 `cursor`、`claude_code`、`codex`、`omp`、`opencode`、`openclaw`、`hermes`、`workbuddy` 或 `freebuff`，便于过滤和追踪来源；已存的 `pi` 记录仍按历史来源显示。
...
<Callout type="warn" icon={false}>
  ...
  普通请求需要额外记忆时，由随 Hook 安装的 Skill 执行按需搜索；其他六个接入会在普通请求前自动注入召回结果。
</Callout>
...
### OMP：Pi 兼容运行时原生 Extension

| 事件 | 时机 | 行为 |
| --- | --- | --- |
| `before_agent_start` | Prompt 展开后、Agent 循环前 | 打开共享 Memmy 会话、启动回合并注入召回上下文；Memory 不可用时不阻断 OMP |
| `agent_settled` | 重试、压缩重试和队列续跑全部结束后 | 对一次用户运行只采集一次最终 Assistant 回答 |
| `input` | 正常 Prompt 展开前 | 将待选择的 `/memmy-resume` 候选转换为携带完整 Episode 上下文的续接请求 |
...
OMP 保留已安装 Pi 兼容运行时的目录和 Extension API，但所有新写入 provenance 使用 `omp`。

### FreeBuff：Skill 与历史扫描

Memmy 扫描 `~/.config/manicode/projects/*/chats/*/chat-messages.json`，并在用户级 `~/.agents/skills/memmy-memory/SKILL.md` 安装共享 Skill。当前 FreeBuff 没有已验证的实时生命周期协议，因此不会安装 Hook 或插件。
...
1. 打开 **记忆管理 → 跨Agent接入**。
2. Cursor、Claude Code、Codex 点击 **安装 Hook**；OMP、OpenCode、OpenClaw、Hermes 点击 **安装插件**；FreeBuff 安装 Skill。
...
点击 **移除 Hook**、**移除插件** 或 **移除 Skill** 时，Memmy 会清理自己管理的接入内容；其他配置会保留，已经进入 Memmy 的历史记忆不会被删除。
