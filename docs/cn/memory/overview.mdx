...
2. workspace 能解析到项目时，Memory 服务把该项目已确认的目标、焦点工作项和约束渲染为权威项目上下文。
3. Memory 服务在新 episode 的第一轮判断请求意图，决定是否召回历史记忆以及允许召回哪些层。
4. 查询被整理为语义查询、关键词、短词模式和结构化错误片段。
5. 各召回通道并行返回候选，再按记忆层、通道排名、质量和时间衰减融合。
6. 候选经过相对阈值、去重、MMR 多样性选择和可选的 LLM 过滤。
7. 项目上下文先注入，随后才是历史记忆命中；当前用户请求始终高于两者。
8. 回合结束后，`turn.complete` 写入原始回合和 L1 轨迹，后台任务再继续摘要、反思、评分、向量化和高层记忆演化。
...
workspace 能解析到项目时，`turn.start` 会在召回记忆前先放入独立的 `<memmy_project_context>` 区块。这里仅包含已确认的项目权威状态：当前目标及其约束、选中的工作项和下一步。Agent 来源与 profile 用于记录出处，不会切分项目，因此同一 workspace 中的 Codex、OMP 和其他已安装接入会收到同一份确认状态。Hook 与插件的具体行为见[Agent 来源与扫描](/cn/memory/sources)。
