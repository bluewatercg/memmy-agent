...
description: Learn how Memmy scans local history from nine built-in Agents and which Hooks, plugins, Skills, and live-memory behaviors it installs.
icon: ScanSearch
---
...
| OMP | `${PI_CODING_AGENT_SESSION_DIR:-~/.pi/agent/sessions}/**/*.jsonl` | Native extension (Pi-compatible runtime) |
...
| FreeBuff | `~/.config/manicode/projects/*/chats/*/chat-messages.json` | Skill |
...
| OMP | Native extension | `before_agent_start`, `agent_settled`, `input` | Uses the Pi-compatible runtime to recall memory before a request, capture fully settled turns, and support `/memmy-resume` |
...
| FreeBuff | Skill | `memmy-memory` CLI | Scans existing history and provides on-demand memory access; no unverified live Hook is installed |
| WorkBuddy | Skill | Agent invokes `memmy-memory` | Searches, reads, and writes shared memory on demand |
| Pi | Skill | Agent invokes `memmy-memory` | Searches, reads, and writes shared memory on demand |
| qwenwork | Skill | Agent invokes `memmy-memory` | Searches, reads, and writes shared memory on demand |

### Authoritative project context at turn start

When a Hook or plugin resolves the current workspace to a project, `turn.start` reads that project's confirmed context before historical recall. Memmy renders the active goal, goal constraints, focused work item, next step, and acceptance criteria in a separate `<memmy_project_context>` block, then appends any relevant historical memory.

This distinction is intentional: approved project context is the current operating state, while L1/L2/L3/Skill results are historical evidence. The current user request remains authoritative over both. The project boundary is shared across Agent sources and profiles, so a goal approved from one CLI is available automatically to another CLI working in the same project. A different project or tenant does not inherit it.

Project context governance is performed in the desktop **Project Context Pack** dialog. Goal proposals are not injected until approved, and only one work item is focused at a time.
...
flowchart LR
  ...
  Start --> Authority["Read confirmed project context"]
  Authority --> Recall["Retrieve relevant historical memory"]
  Recall --> Context["Inject project context, then history"]
...
- **Authoritative project context:** supported integrations receive the same approved goal, constraints, and selected focus for the resolved project before historical recall.
- **Automatic recall:** Claude Code, Codex, OMP, OpenCode, OpenClaw, and Hermes retrieve and inject relevant historical memory before normal requests run.
...
- **Source attribution:** captured turns carry `cursor`, `claude_code`, `codex`, `omp`, `opencode`, `openclaw`, `hermes`, `workbuddy`, or `freebuff`; existing stored `pi` records remain visible as historical provenance.
...
<Callout type="warn" icon={false}>
  ...
  When a normal Cursor request needs additional memory, the bundled Skill performs an on-demand lookup. The other six integrations inject recalled context before normal requests.
</Callout>
...
### OMP: native extension on the Pi-compatible runtime

| Event | Timing | Behavior |
| --- | --- | --- |
| `before_agent_start` | After prompt expansion, before the agent loop | Opens the shared Memmy session, starts the turn, and injects recalled context without blocking OMP if Memory is unavailable |
| `agent_settled` | After retries, compaction retries, and queued continuations finish | Captures the final Assistant response exactly once for the user run |
| `input` | Before normal prompt expansion | Converts a pending `/memmy-resume` candidate selection into a continuation request with full episode context |

Written or updated by default:

- `~/.pi/agent/extensions/memmy-memory.ts`
- `~/.pi/agent/extensions/memmy-memory-config.json`
- `~/.pi/agent/AGENTS.md`
- `~/.pi/agent/skills/memmy-memory/SKILL.md`
OMP retains the installed Pi-compatible runtime directory and Extension API, while all new provenance uses `omp`.

### FreeBuff: Skill and history scanning

Memmy scans `~/.config/manicode/projects/*/chats/*/chat-messages.json` and installs the shared Skill at the user-level `~/.agents/skills/memmy-memory/SKILL.md`. FreeBuff currently has no verified live lifecycle protocol, so Memmy does not install a Hook or plugin.
...
2. For Cursor, Claude Code, and Codex, click **Install Hook**. For OMP, OpenCode, OpenClaw, and Hermes, click **Install plugin**. FreeBuff installs the Skill.
