...
export function memoryViewerAsset(pathname: string): ViewerAsset | undefined {
  ...
  if (!root) {
    ...
    return { body: Buffer.from(memoryPanelHtml()), contentType: "text/html; charset=utf-8", cacheControl: "no-store" };
  }
...
const VIEWER_PUBLIC_ASSETS = new Set(["/memos-logo.svg", "/hermes-logo.svg", "/openclaw-logo.svg"]);
const VIEWER_ROOT_ROUTES = new Set(["/overview", "/user-memories", "/memories", "/tasks", "/policies", "/world-models", "/skills", "/analytics", "/logs", "/settings"]);

function assetFromPath(path: string): ViewerAsset {
