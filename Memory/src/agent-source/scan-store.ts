...
export async function openMemoryAgentSourceScanStore(path: string, job: MemoryScanJobMeta): Promise<MemoryAgentSourceScanStore> {
  ...
  const saveTurnMeta = db.prepare("INSERT INTO turn_meta(source_id,conversation_id,turn_id,first_message_id,first_created_at,last_message_id,last_created_at,selected) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(source_id,conversation_id,turn_id) DO UPDATE SET first_message_id=excluded.first_message_id,first_created_at=excluded.first_created_at,last_message_id=excluded.last_message_id,last_created_at=excluded.last_created_at,selected=excluded.selected");
  const getTurnMeta = db.prepare("SELECT source_id AS sourceId,conversation_id AS conversationId,turn_id AS turnId,first_message_id AS firstMessageId,first_created_at AS firstCreatedAt,last_message_id AS lastMessageId,last_created_at AS lastCreatedAt,selected FROM turn_meta WHERE source_id=? AND conversation_id=? AND turn_id=?");
  const saveResult = db.prepare("INSERT INTO scan_results(source_id,conversation_id,memory_id,error) SELECT ?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM scan_results WHERE source_id=? AND conversation_id=? AND memory_id IS ? AND error IS ?)");
  const store: MemoryAgentSourceScanStore = {
    ...
    saveTurnMeta(m){saveTurnMeta.run(m.sourceId,m.conversationId,m.turnId,m.firstMessageId,m.firstCreatedAt,m.lastMessageId,m.lastCreatedAt,m.selected?1:0);},
    getTurnMeta(s,c,t){const row=getTurnMeta.get(s,c,t) as (Omit<PreparedTurn,"selected">&{selected:number})|undefined; return row?{...row,selected:row.selected===1}:null;},
    ...
    saveResult(r){saveResult.run(r.sourceId,r.conversationId,r.memoryId??null,r.error??null,r.sourceId,r.conversationId,r.memoryId??null,r.error??null);}, resultCount(s){const row=db.prepare(`SELECT COUNT(*) AS count FROM scan_results${s?" WHERE source_id=?":""}`).get(...(s?[s]:[])) as {count:number}; return Number(row.count);},
