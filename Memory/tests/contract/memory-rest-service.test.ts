...
describe("MemoryService / REST contract", () => {
  ...
  it("enforces topic inbox namespace, scopes, idempotency, and bounded evidence", async () => {
    ...
    await withServerClosed(server, async () => {
      ...
      let decisions = 0;
      service.decideProjectTopicCandidate = async (_namespace, _id, decision) => {
        decisions += 1;
        const audit = new Repositories(db.db).runtime.insertAudit({ userId: namespace.userId, actor: decision.actor ?? {}, action: "topic_candidate_rejected", targetKind: "topic_candidate", targetId: "candidate-1", meta: {}, createdAt: "2026-08-11T00:00:00.000Z" });
