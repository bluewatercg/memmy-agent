...
export function openSse(
  ...
  function emit(event: string, data: string, id?: string) {
    ...
    if (event === "memory.changes") {
      for (const mapped of memmyCoreEvents(data)) {
        handler(mapped.type, JSON.stringify(mapped), id);
      }
      return;
    }
...
function memmyCoreEvents(data: string): Array<{
  type: string;
  ts: number;
  seq: number;
  correlationId?: string;
  payload: MemmyPanelChange;
}> {
  let payload: unknown;
  try {
  ...
  return changes
    .filter((change): change is MemmyPanelChange => !!change && typeof change === "object")
    .map((change, index) => ({
      type: memmyCoreEventType(change),
      ts: Date.parse(change.updatedAt ?? "") || Date.now(),
      seq: typeof change.seq === "number" ? change.seq : index,
      ...(change.id ? { correlationId: change.id } : {}),
      payload: change,
    }));
}

function memmyCoreEventType(change: MemmyPanelChange): string {
  if (change.kind === "policy") return change.op === "created" ? "l2.candidate_added" : "l2.revised";
  if (change.kind === "world_model") return change.op === "created" ? "l3.abstracted" : "l3.revised";
  if (change.kind === "skill") {
    if (change.op === "created") return "skill.crystallized";
    if (change.op === "archived" || change.op === "deleted") return "skill.archived";
    return "skill.eta_updated";
  }
  if (change.kind === "episode") return change.op === "created" ? "episode.opened" : "episode.closed";
  if (change.kind === "session") return change.op === "created" ? "session.opened" : "session.closed";
  if (change.kind === "feedback") return "feedback.received";
  if (change.kind === "recall") return "retrieval.triggered";
  if (change.kind === "trace" || change.kind === "span" || change.kind === "user_memory") {
    return change.op === "created" ? "trace.created" : "trace.value_updated";
  }
  return "system.config_changed";
}
