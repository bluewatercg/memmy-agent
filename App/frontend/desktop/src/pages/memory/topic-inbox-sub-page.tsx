...
export function TopicInboxSubPage({ client, projects }: TopicInboxSubPageProps) {
  ...
  async function load(selected = projectId) {
    ...
    try {
      ...
      setTopics(output.projects.flatMap((group) => group.namespace.projectId === selected ? group.topics : []));
      setState("ready");
    } catch (error) {
  ...
  async function refresh() {
    ...
    try {
      ...
      setMessage(result.unchanged ? "Already up to date" : "Refresh queued");
      await load();
  ...
  async function merge(topic: TopicInboxTopic) {
    if (!client) return;
    const target = topics.find((item) => item.id !== topic.id && item.status === "active");
    if (!target) { setMessage("No merge target is available"); return; }
    setBusy(topic.id);
    ...
    catch (error) { setMessage(error instanceof Error ? error.message : String(error)); await load(); }
    finally { setBusy(null); }
  }

  async function split(topic: TopicInboxTopic) {
    const loaded = evidence[topic.id];
    if (!client || !loaded || typeof loaded === "string" || !loaded.items[0]) { setMessage("Expand evidence before splitting"); return; }
    setBusy(topic.id);
    try { await client.splitTopic(topic.id, { ...mutation(projectId), expectedVersion: topic.version, title: `${topic.title} split`, summary: "", evidenceMemoryIds: [loaded.items[0].memoryId] }); await load(); }
    catch (error) { setMessage(error instanceof Error ? error.message : String(error)); await load(); }
    finally { setBusy(null); }
  }
  ...
  return <section className="mx-auto w-full max-w-6xl px-6" aria-labelledby="topic-inbox-title">
    <header className="mb-5 flex flex-wrap items-center gap-3">
      ...
      <button type="button" title="Refresh topic analysis" aria-label="Refresh topic analysis" disabled={!projectId || busy === "refresh"} onClick={() => void refresh()} className="sidebar-toolbar-button"><RefreshCw size={17} className={busy === "refresh" ? "animate-spin" : ""} /></button>
    </header>
    ...
    {state === "ready" && topics.length > 0 && <div><div className="mb-2 text-xs font-medium uppercase text-text-ink/50">Pending candidates: {pendingCount}</div>{topics.map((topic) => {
      ...
      return <article key={topic.id} className="border-t border-border py-3">
        <div className="flex items-start gap-2"><button type="button" aria-expanded={open} aria-label={`${open ? "Collapse" : "Expand"} ${topic.title}`} onClick={() => setExpanded((current) => { const next = new Set(current); open ? next.delete(topic.id) : next.add(topic.id); return next; })} className="mt-1 p-1">{open ? <ChevronDown size={16}/> : <ChevronRight size={16}/>}</button><div className="min-w-0 flex-1"><h2 className="font-medium">{topic.title}</h2><p className="text-sm text-text-ink/65">{topic.summary}</p><div className="mt-1 text-xs text-text-ink/50">{topic.evidenceCount} evidence · {topic.candidateCounts.pending} pending · {topic.status}</div></div><button title="Merge topic" aria-label={`Merge ${topic.title}`} disabled={busy === topic.id} onClick={() => void merge(topic)}><GitMerge size={16}/></button><button title="Split topic" aria-label={`Split ${topic.title}`} disabled={busy === topic.id} onClick={() => void split(topic)}><GitPullRequest size={16}/></button></div>
        {open && <div className="ml-9 mt-3 space-y-2">{topic.candidates.filter((candidate) => candidate.status === "pending" || candidate.status === "deferred").map((candidate) => <Candidate key={candidate.id} candidate={candidate} busy={busy === candidate.id} editing={editing === candidate.id} onEdit={() => setEditing(candidate.id)} onDecide={decide} />)}<button type="button" aria-expanded={Boolean(loaded)} onClick={() => void toggleEvidence(topic.id)} className="text-sm underline">{loaded ? "Hide raw evidence" : `Show raw evidence (up to ${EVIDENCE_LIMIT})`}</button>{loaded === "loading" && <p aria-busy="true">Loading evidence...</p>}{loaded === "error" && <p role="alert">Could not load evidence.</p>}{loaded && typeof loaded !== "string" && <div className="border-l-2 border-border pl-3">{loaded.items.map((item) => <details key={item.id}><summary>{item.role}: {item.summary}</summary><pre className="max-h-48 overflow-auto whitespace-pre-wrap text-xs">{item.rawText}</pre></details>)}</div>}</div>}
      </article>;
    })}</div>}
  </section>;
}
