...
describe("local api", () => {
  ...
  it("exposes the built-in agent sources in registry order", async () => {
    backend = await createTempBackend();
    ...
    await expect(response.json()).resolves.toEqual([
      ...
      expect.objectContaining({ sourceId: "omp", displayName: "OMP" }),
      ...
      expect.objectContaining({ sourceId: "freebuff", displayName: "FreeBuff" })
    ]);
  });
});
